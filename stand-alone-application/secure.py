import ENCDEC
import time
import unicodedata
import os
import os.path
import DH
import codecs
from DH import generate_secret
from ENCDEC import AESCipher
import binascii

global key
global prime_


def encrypt(filename, directory, public_key, private_key):
    # Generate the shared secret using the Diffie-Hellman algorithm
    key = generate_secret(int(private_key), int(public_key))
    
    # Convert the shared secret to bytes
    key_bytes = bytes.fromhex(key)
    
    # Use only the first 32 bytes for the AES key
    aes_key = key_bytes[:32]

    with open(filename, "rb") as file_obj:
        start_time = time.time()
        
        # Encrypt the file content using AES
        encrypted_msg = AESCipher(aes_key).encrypt(file_obj.read())
        
        end_time = time.time()

    # Use a part of the key for the output filename
    output_filename = os.path.join(directory, key[:16] + ".txt")

    with open(output_filename, 'wb') as file_obj:
        file_obj.write(encrypted_msg)

    # Optionally, remove the original file
    os.remove(filename)

    # Open the directory (use os.startfile for Windows)
    os.startfile(directory)

    # Print the encryption time for debugging
    print(f"Encryption took {end_time - start_time} seconds")

def decrypt(filename, directory, public_key, private_key):
    key = DH.generate_secret(int(private_key), int(public_key))
    hex_key = codecs.encode(key, 'hex')[:32]  # Use only the first 32 characters
    key = binascii.unhexlify(hex_key)  # Convert the hex string to bytes
    with open(filename, "rb") as file_obj:
        msg = file_obj.read()

    text = ENCDEC.AESCipher(key).decrypt(msg)

    outputFilename = os.path.join(directory, os.path.basename(filename))
    with open(outputFilename, "wb") as file_obj:
        file_obj.write(text)
    os.remove(filename)
    os.system(f"xdg-open {directory}")
    
def public_key_to_openssh(key):
    """Convert a public key in integer format to OpenSSH format"""
    key_str = str(key)
    key_parts = key_str.split(' ')
    if len(key_parts) != 2:
        raise ValueError("Invalid public key")
    modulus, exponent = key_parts
    return f"ssh-rsa {exponent} {modulus}"
def validate_public_key(public_key):
    """Validate a public key."""
    if not isinstance(public_key, int) or public_key <= 0:
        return False
    return True

def validate_private_key(private_key):
    """Validate a private key."""
    if not isinstance(private_key, int) or private_key <= 0:
        return False
    return True

prime_ = 
