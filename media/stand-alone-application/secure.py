import ENCDEC
import time
import unicodedata
import os
import os.path
import DH

global key
global prime_


import ENCDEC
import time
import os
import DH
import codecs

def encrypt(filename, directory, public_key, private_key):
    # Generate the shared secret key
    key = DH.generate_secret(int(private_key), int(public_key))
    
    # Ensure the key is bytes before encoding
    key_bytes = key.encode('utf-8')
    
    # Encode the key in hex format and use only the first 32 characters
    hex_key = codecs.encode(key_bytes, 'hex')[:32]
    
    # Open the file to be encrypted
    with open(filename, "rb") as file_obj:
        file_data = file_obj.read()

    # Encrypt the file data
    encrypted_data = ENCDEC.AESCipher(hex_key).encrypt(file_data)

    # Create the output filename and write the encrypted data to the file
    output_filename = os.path.join(directory, hex_key[:16].decode('utf-8') + ".txt")
    with open(output_filename, 'wb') as file_obj:
        file_obj.write(encrypted_data)

    # Remove the original file
    os.remove(filename)
    os.system(f"xdg-open {directory}")

def decrypt(filename, directory, public_key, private_key):
    # Generate the shared secret key
    key = DH.generate_secret(int(private_key), int(public_key))
    
    # Ensure the key is bytes before encoding
    key_bytes = key.encode('utf-8')
    
    # Encode the key in hex format and use only the first 32 characters
    hex_key = codecs.encode(key_bytes, 'hex')[:32]
    
    # Open the file to be decrypted
    with open(filename, "rb") as file_obj:
        encrypted_data = file_obj.read()

    # Decrypt the file data
    decrypted_data = ENCDEC.AESCipher(hex_key).decrypt(encrypted_data)

    # Create the output filename and write the decrypted data to the file
    output_filename = os.path.join(directory, "DecodedFile.txt")
    with open(output_filename, 'wb') as file_obj:
        file_obj.write(decrypted_data)

    # Remove the original file
    os.remove(filename)
    os.system(f"xdg-open {directory}")

prime_ = 